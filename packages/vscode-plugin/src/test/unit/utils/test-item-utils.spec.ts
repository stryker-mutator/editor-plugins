...
import { MutantResult, FileRange, MutationTestParams } from 'mutation-server-protocol';
...
describe('testItemUtils', () => {
  ...
  beforeEach(() => {
    testController = vscode.tests.createTestController('test-item-utils', 'Test Item Utils');
  ...
  describe('isMutantInTestTree', () => {
    it('should return true when mutant exists in test tree', () => {
      ...
      const mutantTestItem = testController.createTestItem(expectedMutantId, 'String Literal Mutant');
    ...
    it('should return false when mutant does not exist in test tree', () => {
      ...
      const mutantTestItem = testController.createTestItem(differentMutantId, 'Number Literal Mutant');
      const testItems = [mutantTestItem];
    ...
    it('should find mutant in deeply nested structure', () => {
      ...
      const mutantItem = testController.createTestItem(expectedMutantId, 'Arithmetic Mutant');
    ...
    it('should handle mutant with no replacement', () => {
      ...
      const mutantTestItem = testController.createTestItem(expectedMutantId, 'Block Statement Mutant');
      const testItems = [mutantTestItem];
  ...
  describe('toMutationTestParams', () => {
    it('should convert test items with URI and range to FileRange objects', () => {
      ...
      const testItem = testController.createTestItem('test-item', 'Test Item', uri);
      ...
      expect(lstatSyncStub.calledOnceWith('/test/project/src/file.ts')).to.be.true;
    });
    ...
    it('should handle directory test items by appending slash', () => {
      ...
      const testItem = testController.createTestItem('src-folder', 'Source Folder', uri);
    ...
    it('should throw error for test item without URI', () => {
      const testItem = testController.createTestItem('no-uri-item', 'No URI Item');
      // No URI set

      expect(() => {
        testItemUtils.toMutationTestParams([testItem]);
      }).to.throw('Test item No URI Item does not have a URI. Cannot run mutation tests on it.');
    });
    ...
    it('should handle mix of files and directories', () => {
      ...
      const fileItem = testController.createTestItem('file-item', 'File Item', fileUri);
      fileItem.range = fileRange;
      const dirItem = testController.createTestItem('dir-item', 'Directory Item', dirUri);

      lstatSyncStub.withArgs('/test/project/file.ts').returns({ isDirectory: () => false });
      lstatSyncStub.withArgs('/test/project/src').returns({ isDirectory: () => true });
    ...
    it('should handle Windows-style paths', () => {
      ...
      const testItem = testController.createTestItem('windows-item', 'Windows Item', windowsUri);

      lstatSyncStub.returns({ isDirectory: () => false });

      const result = testItemUtils.toMutationTestParams([testItem]);

      expect(result.files![0].path).to.equal('c:\\Users\\test\\project\\file.ts');
    });
  });
});
